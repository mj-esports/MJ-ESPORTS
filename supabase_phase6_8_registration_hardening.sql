-- ============================================================================
-- MJ ESPORTS — Phase 6.8: Tournament Registration Hardening Migration
-- Target Engine: Supabase PostgreSQL
-- Description: 1. Revokes direct client INSERT privileges on tournament_registrations
--                 and tournament_players tables.
--              2. Replaces permissive INSERT RLS policies with admin-only policies.
--              3. Updates public.register_tournament_team SECURITY DEFINER function
--                 to omit sensitive PII (email, whatsappNumber, captainDob, playerAge)
--                 from public teams_list JSONB entries.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. HARDEN TOURNAMENT_REGISTRATIONS INSERT PRIVILEGES & RLS POLICIES
-- ----------------------------------------------------------------------------

-- Remove insecure direct authenticated user INSERT policy
DROP POLICY IF EXISTS "Authenticated users register" ON public.tournament_registrations;
DROP POLICY IF EXISTS "Admins insert registrations" ON public.tournament_registrations;

-- Create strict admin-only INSERT policy for defensive database boundary
CREATE POLICY "Admins insert registrations"
  ON public.tournament_registrations FOR INSERT
  WITH CHECK (public.is_admin());

-- Revoke direct client INSERT privileges
REVOKE INSERT ON public.tournament_registrations FROM anon, authenticated, public;


-- ----------------------------------------------------------------------------
-- 2. HARDEN TOURNAMENT_PLAYERS INSERT PRIVILEGES & RLS POLICIES
-- ----------------------------------------------------------------------------

-- Remove insecure direct authenticated user INSERT policy
DROP POLICY IF EXISTS "Authenticated users insert player records" ON public.tournament_players;
DROP POLICY IF EXISTS "Admins insert player records" ON public.tournament_players;

-- Create strict admin-only INSERT policy
CREATE POLICY "Admins insert player records"
  ON public.tournament_players FOR INSERT
  WITH CHECK (public.is_admin());

-- Revoke direct client INSERT privileges
REVOKE INSERT ON public.tournament_players FROM anon, authenticated, public;


-- ----------------------------------------------------------------------------
-- 3. UPDATE REGISTER_TOURNAMENT_TEAM RPC (SANITIZED PUBLIC TEAMS_LIST JSONB)
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- 7. Prepare reference ID and insert into public.tournament_registrations (FULL PRIVATE PII RECORD)
  ...
  -- 9. Build SANITIZED JSON object (PII REMOVED: email, whatsappNumber, captainDob, playerAge)
