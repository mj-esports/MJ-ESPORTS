    ...
    <nav className="sticky top-0 z-50 bg-[#111417]/85 backdrop-blur-md border-b border-[#3a494b]/60 shadow-[0_8px_24px_rgba(0,0,0,0.5)]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-18">
          ...
          {/* Desktop Nav Links */}
          <div className="hidden md:flex items-center gap-1 bg-[#1d2023] p-1.5 rounded-full border border-[#3a494b]/60 shadow-inner">
            {navLinks.map((link) => (
              <Link
                key={`nav-desktop-${link.name}`}
                to={link.path}
                className={`px-5 py-2 text-xs font-headline font-semibold uppercase tracking-wider rounded-full transition-all duration-200 ${
                  isActive(link.path)
                    ? 'bg-[#00f2ff] text-[#00363a] font-bold shadow-[0_0_12px_rgba(0,242,255,0.5)]'
                    : 'text-[#b9cacb] hover:text-[#00f2ff] hover:bg-[#282a2e]'
                }`}
              >
                {link.name}
              </Link>
            ...
            {isAuthenticated ? (
              <div className="flex items-center gap-3">
                ...
                {isAdmin && (
                  <Link
                    to="/admin"
                    className="px-3.5 py-1.5 rounded-md bg-[#fe6b00]/10 border border-[#fe6b00]/40 text-[#fe6b00] hover:bg-[#fe6b00]/20 text-xs font-headline font-bold flex items-center gap-1.5 transition-colors uppercase tracking-wider shadow-sm"
                  >
                ...
                <Link
                  to="/profile"
                  className="px-3.5 py-1.5 rounded-md bg-[#1d2023] border border-[#3a494b] hover:border-[#00f2ff] flex items-center gap-2 transition-colors shadow-sm"
                >
                  ...
                  {isAdmin && (
                    <span className="px-2 py-0.5 text-[9px] font-headline font-extrabold bg-[#fe6b00] text-slate-950 rounded uppercase shadow-sm">
                      ADMIN
                    </span>
                  )}
                </Link>
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-md bg-[#1d2023] border border-[#3a494b] text-xs font-headline font-semibold text-[#b9cacb] hover:text-[#ef4444] hover:border-[#ef4444]/40 transition-colors flex items-center gap-1.5 uppercase disabled:opacity-50 min-h-[36px]"
                >
                  {isSigningOut ? (
          ...
          <div className="md:hidden">
            <button
              ...
              className="p-2.5 rounded-md bg-[#1d2023] border border-[#3a494b] text-[#e1e2e7] hover:text-[#00f2ff] focus:outline-none min-h-[44px] min-w-[44px] flex items-center justify-center"
